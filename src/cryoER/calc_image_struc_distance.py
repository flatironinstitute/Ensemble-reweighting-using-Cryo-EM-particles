...
def main(args=None):
    ...
    N_center = len(
        uStr.trajectory
    )  ## Number of centers, i.e., representative configuration from MD
    ...
    if n_batch == 1:
        ...
        rot_mats = torch.from_numpy(
            np.load("%s/rot_mats_%s.npy" % (outdir, file_prefix))
        )
        ...
        for i in tqdm(range(n_struc)):
            aligned_coord = (
                coord_str[i].unsqueeze(0).matmul(rot_mats_align[i]).matmul(rot_mats)
            )
            if device == "cuda":
        ...
        for i_batch in range(n_batch):
            ...
            np.save(
                "%s/rot_mats_%s_batch%d.npy" % (outdir, file_prefix, i_batch), rot_mats
            )
            ...
            rot_mats = torch.from_numpy(
                np.load("%s/rot_mats_%s_batch%d.npy" % (outdir, file_prefix, i_batch))
            )
            ctfs = torch.from_numpy(
                np.load("%s/ctf_%s_batch%d.npy" % (outdir, file_prefix, i_batch))
            )
            images = torch.from_numpy(
                np.load("%s/images_%s_batch%d.npy" % (outdir, file_prefix, i_batch))
            )
            ...
            for i in tqdm(range(n_struc)):
                aligned_coord = (
                    coord_str[i]
                    .unsqueeze(0)
                    .matmul(rot_mats_align[i, batch_start:batch_end])
                    .matmul(rot_mats)
                )
                if device == "cuda":
